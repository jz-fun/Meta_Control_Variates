...
class MetaNeuralCVModel_Sarcos(torch.nn.Module):
    def __init__(self, D_in,  h_dims, init_val):

        super(MetaNeuralCVModel_Sarcos, self).__init__()
    ...
    def forward(self, x, score_x):
        score = score_x.float()
        eva_net, grads = self.net_utils(x.float())

        y_pred = self.c + (grads.sum() + eva_net * score.sum())/1e20
...
class MetaNeuralCV_Sarcos():
    ...
    def main_loop(self, num_iterations):
        ...
        for each_par in self.net.parameters():   ## initialize this neural nets
            ...
            torch.nn.init.normal_(each_par, mean=0, std=1e-3)
    ...
    def test(self, test_tasks_distr,n_total_tasks , inner_optim, inner_steps=3):
        self.log_test = []
        ...
        for i in range(n_total_tasks):
            print(i)
            task = test_tasks_distr.sample_task()
