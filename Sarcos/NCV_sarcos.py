...
class NeuralCVModel_Sarcos(torch.nn.Module):
    def __init__(self, D_in,  h_dims, init_val):
        ...
        :param init_val:
        """
        super(NeuralCVModel_Sarcos, self).__init__()
...
class NeuralCV_Sarcos():
    ...
    def train_val(self, num_epochs, batch_size, verbose = True):
        ...
        for i in range(num_epochs):
            ...
            if train_perc < 1.0:
                val_ncv_est = ( Y_qry.detach() - self.net.minibatch(X_qry,Score_X_qry).detach()+self.net.c.detach()).mean().numpy()
                if verbose:
